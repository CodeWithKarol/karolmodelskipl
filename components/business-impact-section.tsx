      ...
      <div className="relative z-10 container mx-auto max-w-7xl px-4">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
          {impacts.map((item, index) => (
            <div
              key={index}
              className={`group relative flex flex-col overflow-hidden rounded-[2rem] border border-border bg-muted/50 p-8 backdrop-blur-sm transition-all duration-500 hover:-translate-y-1 hover:border-primary/30 hover:bg-muted/80 sm:p-10 ${
                index === 2 ? "md:col-span-2 lg:col-span-1" : ""
              }`}
            >
              {/* Efekt podświetlenia karty od góry */}
              <div className="absolute top-0 left-0 h-1.5 w-full bg-gradient-to-r from-transparent via-primary/0 to-transparent transition-all duration-500 group-hover:via-primary/50"></div>

              <div className="pointer-events-none absolute top-0 right-0 h-32 w-32 rounded-full bg-primary/10 opacity-0 blur-[40px] transition-opacity duration-500 group-hover:opacity-100"></div>
