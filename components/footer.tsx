  ...
  return (
    <footer className="relative overflow-hidden border-t border-slate-900 bg-slate-950 py-12 text-slate-400 md:py-20">
      ...
      <div className="relative z-10 container mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 xl:grid-cols-12 xl:gap-12">
          ...
          {/* Kolumna 3 & 4 zagnieżdżone: Szybka nawigacja */}
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:col-span-2 xl:col-span-5">
            {/* Kolumna 3 */}
              ...
              <nav className="flex flex-col space-y-3.5">
                {footer.col3.navigation.map((nav, idx) => (
                  <Link
                    key={idx}
                    href={nav.href}
                    className="group w-fit text-[15px] text-slate-400 transition-colors hover:text-white"
                  >
                    {nav.label}
                  </Link>
                ))}
            ...
            <div className="flex flex-col gap-5">
              ...
              <nav className="flex flex-col space-y-3.5">
                {footer.col4.navigation.map((nav, idx) => (
                  <Link
                    key={idx}
                    href={nav.href}
                    className="group w-fit text-[15px] text-slate-400 transition-colors hover:text-white"
                ...
                {footer.col4.links.map((link, idx) =>
                  link.isCta ? (
