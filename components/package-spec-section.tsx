...
function TierGrid({ tiers }: { tiers: { label: string; text: string }[] }) {
  return (
    <ul className="mt-5 space-y-3">
      {tiers.map((tier) => (
        <li key={tier.label} className="flex items-start gap-2.5">
          <span
            aria-hidden="true"
...
export function PackageSpecSection({
  ...
  return (
    <section
      className={cn(
        "relative overflow-hidden border-t border-border bg-background py-12 text-muted-foreground sm:py-20 md:py-24",
        className
      )}
    >
      ...
      <div className="relative z-10 container mx-auto px-4 max-w-4xl">
        ...
        <div className="space-y-0">
          ...
          <Reveal>
              ...
              {spec.core.bullets && spec.core.bullets.length > 0 && (
                <BulletList bullets={spec.core.bullets} />
              )}
            ...
            <div className="border-t border-border py-7 sm:py-9">
                ...
                <p className="mt-3 max-w-prose text-sm leading-relaxed text-muted-foreground sm:text-[15px]">
                  ...
                  {bonus.bullets && bonus.bullets.length > 0 && (
                    <BulletList bullets={bonus.bullets} />
                  )}
