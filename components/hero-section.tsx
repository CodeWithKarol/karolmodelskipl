    ...
    <section className="relative overflow-hidden bg-slate-950 text-slate-50 min-h-[100dvh] flex items-center justify-center selection:bg-blue-500/30">
      ...
      <div className="container relative z-10 mx-auto px-4 py-16 md:py-32 max-w-7xl">
        <div className="flex flex-col items-center text-center max-w-5xl mx-auto space-y-8 md:space-y-10">
          ...
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-[5rem] font-extrabold tracking-tight text-white leading-tight sm:leading-[1.05] text-balance">
          ...
          <p className="text-base sm:text-lg md:text-2xl leading-relaxed text-slate-300 text-balance max-w-3xl font-light">
            {hero.subheading}
          </p>
          
          {/* Przyciski i Call To Action */}
          <div className="w-full max-w-md mx-auto pt-4 md:pt-8 flex flex-col items-center space-y-4">
            <a 
            ...
            >
              <div className="relative w-full">
                ...
                <Button 
                  size="lg" 
                  className="relative w-full h-auto min-h-[3.5rem] py-3 sm:py-0 sm:h-16 text-[15px] sm:text-lg font-semibold bg-blue-600 hover:bg-blue-500 text-white rounded-2xl shadow-xl transition-all flex flex-row items-center justify-center gap-2 sm:gap-3 border border-blue-400/20 whitespace-normal text-center leading-snug px-5 sm:px-8"
                >
            ...
            <p className="text-[11px] sm:text-sm text-slate-500 font-medium tracking-wide flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 mt-3 sm:mt-2 text-center">
          ...
          <div className="mt-10 md:mt-24 pt-6 sm:pt-8 border-t border-slate-800/80 w-full max-w-4xl mx-auto">
            <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-8">
              {hero.indicators.map((ind, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2 sm:gap-3 text-slate-300 bg-slate-900/50 sm:bg-transparent border border-slate-800/60 sm:border-none rounded-full px-4 py-2 sm:p-0">
                    <Icon className="h-4 w-4 sm:h-6 sm:w-6 text-blue-400 shrink-0" />
                    <span className="text-[13px] sm:text-base font-medium">{ind.text}</span>
                  </div>
                );
              })}
            </div>
