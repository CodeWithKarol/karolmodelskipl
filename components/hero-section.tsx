      ...
      <div className="relative z-10 flex-1 container mx-auto px-4 flex flex-col lg:flex-row items-center justify-start lg:justify-between gap-8 sm:gap-10 lg:gap-8">
        ...
        <div className="flex-1 w-full max-w-2xl text-left lg:pr-8">
          <div className="mb-4 sm:mb-6 animate-[fade-up_0.7s_ease-out_both]">
            <SectionBadge className="border-white/10 bg-white/5 text-slate-400 text-[10px] sm:text-xs">
          ...
          <p className="text-base sm:text-lg text-slate-400 mb-6 sm:mb-8 leading-relaxed max-w-xl animate-[fade-up_0.7s_ease-out_0.2s_both]">
            {hero.subheading}
          </p>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2.5 sm:gap-x-6 animate-[fade-up_0.7s_ease-out_0.3s_both]">
            {hero.indicators.map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-left text-slate-300">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mt-6 sm:mt-8 animate-[fade-up_0.7s_ease-out_0.35s_both]">
            <a href="#zwrotnica" className="w-full sm:w-auto">
              <Button className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 text-sm font-bold rounded-xl shadow-lg transition-all whitespace-normal h-auto leading-tight">
                Wybierz wyzwanie w Twojej firmie ↓
              </Button>
            </a>
            <span className="text-xs text-slate-400 text-center sm:text-left">lub sprawdź doświadczenie poniżej</span>
          </div>
        </div>
        ...
        <div className="relative w-full max-w-[21rem] sm:max-w-sm lg:max-w-md flex-shrink-0 mx-auto lg:mx-0 lg:pr-4 animate-[fade-in_0.7s_ease-out_0.4s_both] lg:animate-[fade-in_0.9s_ease-out_0.15s_both]">
          {/* Zdjęcie */}
          <div className="relative w-full rounded-[2rem] overflow-hidden shadow-[0_25px_80px_-20px_rgba(37,99,235,0.25)] border border-slate-800">
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent z-10 pointer-events-none"></div>
            ...
            <div className="absolute bottom-0 left-0 right-0 z-20 p-5 sm:p-6">
              <p className="text-sm sm:text-base font-bold text-white">Karol Modelski</p>
              <p className="text-[11px] sm:text-xs text-blue-400 font-medium mb-3">Architekt Oprogramowania &amp; Niezależny Partner Technologiczny</p>

              <div className="pt-3 border-t border-white/10 flex items-center justify-between gap-3 text-[10px] sm:text-[11px] text-slate-300">
                <span className="shrink-0">Standardy bankowe:</span>
                <span className="font-semibold text-slate-100 text-right">Citibank • BNP Paribas</span>
              </div>
            </div>
          </div>

          {/* Tła poświaty */}
          <div className="absolute -top-6 -left-2 w-40 h-40 rounded-full bg-blue-500/10 blur-3xl pointer-events-none"></div>
          <div className="absolute -bottom-6 -right-2 w-32 h-32 rounded-full bg-blue-600/10 blur-3xl pointer-events-none"></div>
        </div>
      </div>
    </section>
  )
