...
export default function ModernizacjaOprogramowaniaPage() {
  ...
  return (
    <main className="flex-1 overflow-hidden bg-slate-950 text-slate-50">
      ...
      <section className="relative flex flex-col items-center justify-center overflow-hidden border-b border-slate-900/80 bg-slate-950 py-24 text-center">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900/20 via-slate-950 to-slate-950"></div>
        <div className="relative z-10 container mx-auto px-4 max-w-4xl">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/5 px-4 py-1.5 text-xs font-bold text-blue-400 uppercase tracking-widest">
            {hero.badge}
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-extrabold tracking-tighter text-white mb-8 leading-[1.05]">
            {hero.title}
            <span className="block text-blue-500 mt-2">{hero.titleHighlight}</span>
          </h1>
          <p className="text-lg sm:text-xl text-slate-400 mb-10 leading-relaxed max-w-2xl mx-auto font-light">
            {hero.description}
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12 w-full text-left">
            {hero.indicators.map((item, i) => (
              <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                <div className="text-blue-400 mb-3 text-xl">
                  {i === 0 ? "⚡" : i === 1 ? "🛡️" : "📈"}
                </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-md w-full relative mx-auto">
            <p className="text-sm sm:text-base font-bold text-blue-400 mb-2">
              👉 Masz wątpliwości, czy Twój system da się uratować?
            </p>
            ...
            {story.points.map((point, idx) => (
              <div key={idx} className="group bg-slate-900/40 border border-slate-800 p-6 sm:p-8 rounded-2xl hover:border-slate-700 transition-all duration-300">
                <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-6">
                  ...
                  <div>
                    <h4 className="text-base sm:text-lg font-bold text-white mb-2" dangerouslySetInnerHTML={{ __html: point.title }} />
            ...
            {contrast.comparisons.map((item, i) => (
              <div key={i} className="contents">
                <div className="bg-slate-900/40 border border-slate-800 p-6 sm:p-8 rounded-2xl flex flex-col hover:border-slate-700 transition-all">
                  <div className="text-red-500 mb-3 font-bold text-[10px] uppercase tracking-widest">{contrast.red_ocean_title}</div>
                  <h4 className="text-base sm:text-lg font-bold text-slate-300 mb-3 leading-snug">{item.red.title}</h4>
                ...
                <div className="bg-blue-600/10 border border-blue-500/30 p-6 sm:p-8 rounded-2xl flex flex-col hover:border-blue-500/50 transition-all">
                  ...
                  <h4 className="text-base sm:text-lg font-bold text-white mb-3 leading-snug">{item.blue.title}</h4>
